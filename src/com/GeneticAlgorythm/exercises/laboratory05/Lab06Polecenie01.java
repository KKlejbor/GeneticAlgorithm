...
public class Lab06Polecenie01 {
    ...
    public static ArrayList<Integer> ruletta(Integer[][] populacja,AlgorytmGenetyczny algo){
        ArrayList<Integer> indeksy = new ArrayList<Integer>();
        Double[] wartośćiFunckji = obliczWartości(populacja,algo);
        Double[] prawdopodobieńwstwa = new Double[N];
        Integer[][] nowaPopulacja = new Integer[][];
        double suma = 0;

        for (int i = 0; i < N; i++) {
            wartośćiFunckji[i] = wartośćiFunckji[i] - algo.getA(); 
            suma += wartośćiFunckji[i];
        }

        for (int i = 0; i < N; i++) {
            prawdopodobieńwstwa[i] = wartośćiFunckji[i] / ((int)suma);
        }
